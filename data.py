...
def get_dataset(opt: dict, data_dir, use_lcc: bool = False) -> InMemoryDataset:
  ...
  if ds in ['Cora', 'Citeseer', 'Pubmed']:
    ...
    dataset = WebKB(root=str(path), name=ds, transform=T.NormalizeFeatures())
  elif ds in ['chameleon', 'squirrel']:
    dataset = WikipediaNetwork(root=str(path), name=ds, transform=T.NormalizeFeatures())
  elif ds == 'film':
    dataset = Actor(root=str(path), transform=T.NormalizeFeatures())
  elif ds == 'ogbn-arxiv':
    dataset = PygNodePropPredDataset(name=ds, root=str(path),
