...
def make_symmetric(data):
  ...
  if data.edge_attr is not None:
    ...
    sparse_tensor = torch.sparse_coo_tensor(ApAT_index, ApAT_value, (n, n))
    coalesced_tensor = sparse_tensor.coalesce()
    ei, ew = coalesced_tensor.indices(), coalesced_tensor.values()
  else:
...
def dirichlet_energy(edge_index, edge_weight, n, X):
  ...
  de = torch.spmm(edge_index, edge_weight, n, n, X)
  return X.T @ de
...
class GDCWrapper(GDC):
  ...
  def position_encoding(self, data):
    ...
    sparse_tensor = torch.sparse_coo_tensor(edge_index, edge_weight, (N, N))
    coalesced_tensor = sparse_tensor.coalesce()
    edge_index, edge_weight = coalesced_tensor.indices(), coalesced_tensor.values()
    if self.exact:
